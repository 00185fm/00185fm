...
export const load = async ({ parent, params, locals, cookies }) => {
  ...
  const tags = structuredClone(
    await pb.collection(Collections.Tags).getFullList()
  );
  const form = await superValidate(updateEpisode);
  const episodes = (await parent()).episodes;
  const episode: RecordModel = episodes.find(
    (i) => i.slug.split("/")[1] === params.episode
  );
  return { episode, form, token: cookiesParsed.token, playlists, tags };
};
